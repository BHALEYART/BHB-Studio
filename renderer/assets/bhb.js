...
const BHB = {
  ...
  _showElectronWalletNotice() {
    ...
    modal.innerHTML = `
      ...
      <div class="bhb-modal-box">
        <div class="bhb-modal-header">
          <span>Wallet — Desktop App</span>
          <button class="bhb-modal-close" aria-label="Close">✕</button>
        </div>
        <div class="bhb-modal-body">
          <p style="font-size:0.85rem;line-height:1.6;color:#333;margin-bottom:1rem;">
            Browser-extension wallets (Phantom, Solflare, Ledger) can't inject into
            the desktop app.<br/><br/>
            <strong>All Customizer &amp; Animator features work without a wallet.</strong>
            Wallet connection is only needed to save traits on-chain to your NFT.
          </p>
          <p style="font-size:0.78rem;color:#666;margin-bottom:0.75rem;">
            To use wallet features, visit the web app:
          </p>
          <button id="bhbElectronOpenWeb" style="
            width:100%;padding:0.6rem;
            font-family:'Impact','Arial Black',sans-serif;font-size:0.88rem;letter-spacing:0.06em;
            background:#ffe135;color:#0d0d0d;
            border:3px solid #0d0d0d;border-radius:6px;
            cursor:pointer;
          ">🌐 Open bigheadbillionaires.com</button>
        </div>
      </div>
    `;
    ...
    modal.querySelector('#bhbElectronOpenWeb').addEventListener('click', () => {
      window.electronAPI.openExternal('https://bigheadbillionaires.com');
      modal.remove();
    });
  ...
  showWalletModal() {
    // ── Desktop app: wallet extensions don't inject into Electron ──
    if (window.electronAPI?.isElectron) {
      BHB._showElectronWalletNotice();
      return;
    }
  ...
  initNav() {
    // Tag body so CSS can hide the browser nav bar in desktop mode
    if (window.electronAPI?.isElectron) {
    ...
    // Highlight active nav link (works for both http and file:// protocols)
    const currentPage = window.location.pathname.split('/').filter(Boolean).slice(-2).join('/');
    document.querySelectorAll('.nav-links a').forEach(link => {
      const href = link.getAttribute('href') || '';
      if (!href.startsWith('http') && !href.startsWith('#')) {
        const linkParts = href.replace(/\.\.\//, '').split('/').filter(Boolean);
        if (linkParts.some(p => currentPage.includes(p.replace('index.html', '')))) {
